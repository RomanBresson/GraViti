...
model_backend = checkpoint.config_state_dict.get("model_backend", "catflow")
dataset = checkpoint_dataset if checkpoint_dataset.startswith("BayesianNet") else "QM9NoHydro"

#%%
with_aromatic = ("arom1" in model_name)
all_data, _, _ = load_data(dataset, splits=[1.,0.,0.], data_root="data", with_aromatic=with_aromatic)

from sklearn.model_selection import KFold

kf = KFold(n_splits=5, shuffle=True, random_state=42)

# Extract embeddings + targets ONCE for all_data
loader_all = make_loader(all_data, batch_size=256, shuffle=False, workers=0)
embeddings_all, _, _, targets_all = get_all_embeddings_downstream(
    num_batches=None,
    loader=loader_all,
...
X = embeddings_all.cpu().numpy()
y = targets_all.cpu().numpy()
...
for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X)):
    print(f"\n===== Fold {fold_idx + 1} / 5 =====")
    targets_tr_val = torch.tensor(y[train_idx], dtype=torch.float32)
    targets_test = torch.tensor(y[test_idx], dtype=torch.float32)
    # Split train into train/val
    targets_tr = targets_tr_val[:int(0.9 * len(targets_tr_val))]
    targets_val = targets_tr_val[int(0.9 * len(targets_tr_val)):]

    embeddings_tr_val = torch.tensor(X[train_idx], dtype=torch.float32)
    embeddings_test = torch.tensor(X[test_idx], dtype=torch.float32)
    embeddings_tr = embeddings_tr_val[:int(0.9 * len(embeddings_tr_val))]
    embeddings_val = embeddings_tr_val[int(0.9 * len(embeddings_tr_val)):]
    ...
    if dataset.startswith("BayesianNet"):
        ...
        all_pearson_rs.append(test_pearson_r)
        print(
            f"Fold {fold_idx + 1} metrics: "
            f"MSE={test_mse:.4f}, RMSE={test_rmse:.4f}, Pearson r={test_pearson_r:.4f}"
        )
    else:
        print(f"Fold {fold_idx + 1} MSE: {test_mse:.4f}")

if dataset.startswith("BayesianNet"):
    print(
        f"\nFinal Test metrics for model {model_name} and target {idx_target}: "
        f"MSE={np.mean(all_mses):.4f} +/- {np.std(all_mses):.4f}, "
        f"RMSE={np.mean(all_rmses):.4f} +/- {np.std(all_rmses):.4f}, "
        f"Pearson r={np.mean(all_pearson_rs):.4f} +/- {np.std(all_pearson_rs):.4f}"
    )
else:
    print(f"\nFinal Test MSE for model {model_name} and target {idx_target}: {np.mean(all_mses):.4f} +/- {np.std(all_mses):.4f}")
# %%
