...
def compute_alibi_matrix(seq_len, heads):
    pos = get_relative_positions(seq_len=seq_len)
    slopes = get_alibi_slope(num_heads=heads)
    return pos * slopes


def multi_head_attention_forward_alibi(
    query: Tensor,
    key: Tensor,
    value: Tensor,
    ...
    alibi_matrix: Optional[Tensor] = None
    ...
    if has_torch_function(tens_ops):
        return handle_torch_function(
            multi_head_attention_forward_alibi,
            tens_ops,
            query,
            key,
            value,
    ...
    if True: #if need_weights:
        ...
        alibi_biases = alibi_matrix.unsqueeze(0).expand(bsz, -1, -1, -1).reshape(bsz * num_heads, src_len, src_len).to(attn_output_weights.device)
        attn_output_weights = attn_output_weights + alibi_biases
    ...
    """
    # fast branch, not compatible with alibi
    else:
...
class MultiheadAttentionAlibi(torch.nn.MultiheadAttention):
    ...
    def __init__(
        ...
        device=None,
        dtype=None,
        default_max_size=32 #to compute the default alibi bias matrix
    ) -> None:
        ...
        self.register_buffer("alibi_matrix", compute_alibi_matrix(seq_len=default_max_size, heads=num_heads).to(**factory_kwargs), persistent=False)
    ...
    def forward(
        ...
        src_len = query.size(0)
        if src_len > self.alibi_matrix.size(0):
            self.register_buffer("alibi_matrix", compute_alibi_matrix(seq_len=src_len, heads=self.num_heads).to(self.alibi_matrix.device), persistent=False)
            
        alibi_biases = self.alibi_matrix[:, :src_len, :src_len]

        if not self._qkv_same_embed_dim:
            attn_output, attn_output_weights = multi_head_attention_forward_alibi(
                query,
                key,
                value,
                self.embed_dim,
...
class TransformerEncoderLayerWithAlibi(Module):
    ...
    def forward(
        ...
        if src_key_padding_mask is not None:
            ...
            num_nodes = src_key_padding_mask.long().sum(-1)
        else:
            num_nodes = torch.tensor(sequence.size(1)).expand(sequence.size(0)).to(src.device)
            src_key_padding_mask = torch.zeros(sequence.size(0), sequence.size(1)).bool().to(src.device)
        list_graphs = torch.arange(sequence.size(0)).to(src.device)
        padded_sequence = torch.ones(sequence.size(0), sequence.size(1)+2, sequence.size(2)).to(src.device)
        padded_sequence[:, 1:-1, :] = sequence
        padded_mask = torch.zeros(sequence.size(0), sequence.size(1)+2).bool().to(src.device)
        padded_mask[:, 1:-1] = src_key_padding_mask
        padded_mask[:, 0] = False
        # signal last element is valid
        padded_sequence[list_graphs, num_nodes+1, :] = -1.
        padded_mask[list_graphs, num_nodes+1] = False
        src = padded_sequence
        x = src
